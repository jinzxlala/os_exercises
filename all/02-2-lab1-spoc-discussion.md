...
>   1.初始化内存管理器，其中主要是对gdt的初始化过程。
    2.初始化8259A中断控制器。过程如下：列出所有的中断>>启动master>>启动slave
    3.初始化IDT。
...
> 
```
    1.实现控制台的直接访问（通过某个输出函数）来方便之后的输出调试。
    2.实现简单的bootloader，并能够打印字符。
    3.加载ELF文件。
    4.开始完成kernel,实现内存初始化和加载GDT
    5.实现中断控制器和IDT的初始化。
    6.实现时钟中断控制。
```
